...
public:
  ...
  DA2Network( const char *network_path ) {
    ...
    // 2. CRITICAL FIX: Disable complex graph fusions that break FP16 models on macOS
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_BASIC);

    // 3. Create the session with these safe options
  ...
  DA2Network( const char *network_path, const char *input_layer_name, const char *output_layer_name ) {
    ...
    // 2. CRITICAL FIX: Disable complex graph fusions
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_BASIC);
